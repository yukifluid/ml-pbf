    ...
    def process(self) -> None:
        ...
        for scene_dir in self.raw_paths:
            ...
            for df in reader:
                ...
                data = torch_geometric.data.Data(V=vertex_features, E=edge_features, N=edge_index, y=dp, vol=vol, mid_pos=mid_pos, mid_vel=mid_vel, num_boundary_particles=config.num_boundary_particles, rho_0=config.rho_0, h=config.h, dt=config.dt, num_nodes=config.num_particles)
...
class Simple3D(torch_geometric.data.Dataset):
    def __init__(self, dataset_type: str) -> None:
        self._dataset_type = dataset_type
        self._name = __class__.__name__
        self._url = f"file:./datasets/{self._name}/{self._dataset_type}.zip"
